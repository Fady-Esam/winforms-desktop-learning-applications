...
namespace TaskProject
{
    internal class DBClass
    {
        ...
        public DBClass()
        {
            try
            {
                string connString = "Server =.; DataBase = TaskDB; Integrated Security = True;Timeout=30";
        ...
        public int InsertStoresData(int storeID, int branchID)
        {
            string insertQry = "INSERT INTO ReleationBranchStoresTbl (StoreID, BranchID) VALUES (@SI, @BI)";
            SqlCommand comm = new SqlCommand(insertQry, sqlConn2);
            comm.Parameters.AddWithValue("@SI", storeID);
            comm.Parameters.AddWithValue("@BI", branchID);
            int affectedRows = -1;
            try
            {
            ...
            {
                MessageBox.Show("Something wrong " + ex + " Please try again");
            }
            return affectedRows;
        }
        public int InsertSaveBoxesData(int saveBoxID, int branchID)
        {
            string insertQry = "INSERT INTO ReleationBranchSaveBoxesTbl (saveBoxID, BranchID) VALUES (@SI, @BI)";
            SqlCommand comm = new SqlCommand(insertQry, sqlConn2);
            comm.Parameters.AddWithValue("@SI", saveBoxID);
            comm.Parameters.AddWithValue("@BI", branchID);
            int affectedRows = -1;
            try
            {
